...
for epoch in range(current_epoch, args.n_epochs):
    for i, (original_img, original_img_pose, original_img_joints, target_img, target_img_pose, target_img_joints) in enumerate(trainloader):
        ...
        z = torch.cat((original_img, target_img_pose), 1)
        if torch.cuda.is_available():
            z = z.cuda()

        fake_img = G(z.detach())
        fake_img_original, fake_img_pose = fake_img.split([3, 3], dim = 1)
        ...
        fake_pose_loss = criterionL1(fake_img_pose, target_img_pose)
